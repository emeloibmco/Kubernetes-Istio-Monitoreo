...
Por otro lado, se implementará el operador de Prometheus que permite instalar Grafana para ambientes productivos, con el fin de visualizar y monitorear métricas del clúster de Kubernetes. 
...
- Complemento [container-service/kubernetes-service](https://cloud.ibm.com/docs/cli?topic=cli-install-devtools-manually) para ibmcloud CLI. `ibmcloud plugin install container-service
`.
...
1. Clone este repositorio y configure las variables de entorno del ambiente. Para ello, ubíquese en la carpeta clonada del repositorio y en la ruta ```Kubernetes-Istio-Monitoreo\istio-1.12.4``` y coloque: 
...
Para realizar la instalación de Istio en el clúster se utilizará el perfil para producción de Istio. Para la prueba que se presenta en esta documentación se utiliza la versión ```1.12.4``` para ambientes productivos.

Para ejecutar los comandos, dentro de la capeta clonada ```Kubernetes-Istio-Monitoreo\istio-1.12.4``` muévase con el comando ```cd``` hasta la carpeta ```bin``` que contiene el ejecutable de istio y allí ejecute los siguientes comandos:
<br />
...
2. Instale el ```kiali-operator``` en el namespace que acaba de crear (```kiali-operator```) junto con el servidor de Kiali en el namespace del sistema istio (```istio-system```). Asegúrese de tener instalado el comando helm (puede que deba cambiar de carpeta hasta la ubicación que contiene el ejecutable) y posteriomente coloque el comando:
   ```
   helm install --set cr.create=true --set cr.namespace=istio-system --namespace kiali-operator --repo https://kiali.org/helm-charts --version 1.42.0 kiali-operator kiali-operator 
   ```
   <br />
...
2. El paso siguiente consiste en exponer al exterior el clúster y definir las políticas de acceso. Para ello, se debe configurar la aplicación para aceptar tráfico externo, agregando el Istio Ingress Gateway, que se encargará de gestionar las rutas de nuestro Service Mesh.
...
3. Defina la configuración de enrutamiento donde se especifica a qué servicios se puede acceder desde el exterior. Para ello utilice el archivo destination-rule-all.yaml mediante el comando:
...
Para visualizar y acceder al dashboard de Kiali en la versión para ambientes productivos, puede realizarlo de 2 formas:
<br />

* **Opción 1:**
  
  Vaya a la carpeta ```bin``` (ya que esta carpeta contiene el ejecutable de Kiali) con ```cd bin``` (que se encuentra dentro de la carpeta clonada ```Kubernetes-Istio-Monitoreo\istio-1.12.4```) y coloque el comando:
...
Revise nuevamente la ventana que muestra Kiali y asegúrese de tener seleccionado lo siguiente:
...
8. Espero unos minutos mientras se completa la implementación. Posteriormenete, visualice de nuevo los pods en el namespace creado con el siguiente comando, identifique el pod de grafana y tenga en cuenta su nombre para usarlo más adelante:
```
kubectl get pods -n monitoring
```
<br />
...
11. Una vez cargue la ventana de Grafana deberá colocar el usuario y contraseña para poder acceder al Dashboard. Para este caso utilice:
